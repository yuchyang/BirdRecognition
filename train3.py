...
def train(model_instance, train_source_loader,test_source_loader ,num_iterations, batch_size, optimizer, lr_scheduler, group_ratios):
    ...
    for iter_num in range(num_iterations):
        ...
        if iter_num % 500 == 0:
            ...
            # }, iter_num)
            if iter_num % 5000 == 0:
                torch.save(model_instance, 'D://model//200resnet50_accuracy{0}_c_net'.format(eval_result2['accuracy']))
...
if __name__ == "__main__":
    ...
    net = DANNClassifier(base_net='ResNet101', use_bottleneck=False, bottleneck_dim=256, class_num=200)
    ...
    net.cuda()
    # optimizer = optim.SGD(parameter_list, lr=1.0, momentum=0.9, weight_decay=0.0005, nesterov=True)
    optimizer = optim.Adam(net.parameters(), lr=0.003,weight_decay=0.0005)
    ...
    group_ratios = [param_group["lr"] for param_group in optimizer.param_groups]
    train(net, train_loader, test_loader, 1000000, batch_size=BATCH_SIZE, optimizer=optimizer,
