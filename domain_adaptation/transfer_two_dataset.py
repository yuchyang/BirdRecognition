...
data_loader2 = torch.utils.data.DataLoader(video_data, batch_size=BATCH_SIZE, shuffle=True)
...
def train(model_instance, train_source_loader, train_target_loader, test_target_loader,test_source_loader ,num_iterations, batch_size, optimizer, lr_scheduler, group_ratios):
    ...
    for iter_num in range(num_iterations):
        ...
        inputs_source, labels_source, box = iter_source.next()
        ...
        if iter_num % 500 == 0 and iter_num is not 0:
            ...
            # }, iter_num)
            if len(valid_accuracy_list)%10 is 0 and len(valid_accuracy_list) is not 0:
                plt.savefig('DANN_{0}.png'.format(len(valid_accuracy_list)))


            standard = eval_result2['accuracy']
            model_instance.save_model(c_net_path='DANN_IMAGE200_accuracy{0}_c_net'.format(standard),d_net_path='DANN_IMAGE200_accuracy{0}_d_net'.format(standard))
    print("finish train.")
...
if __name__ == '__main__':
    ...
    model = transfor_net.DANN(base_net='ResNet101', use_bottleneck=True, bottleneck_dim=256, class_num=200, hidden_dim=1024,
    ...
    test_dataset = CUB_200(cub200_root, train=False, transform=transform, target_transform=None)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=True)
    train(model,train_loader,data_loader2,test_loader,test_loader,1000000, batch_size=BATCH_SIZE, optimizer=optimizer, lr_scheduler=scheduler, group_ratios=group_ratios)
