...
class DCDA(object):
    ...
    def get_loss(self, inputs, labels_source, epoch):
        ...
        src_entropy_loss = self.trade_off * loss.entropy_loss((tgt_probabilities_a + tgt_probabilities_b)/2) / batch_size
        tgt_entropy_loss = self.trade_off * loss.entropy_loss((tgt_probabilities_a + tgt_probabilities_b)/2) / batch_size
        self.writer.add_scalars('data/loss', {
            'src_entropy_loss': src_entropy_loss,
            'tgt_entropy_loss': tgt_entropy_loss,
            }, epoch)

        return classifier_loss, classifier_loss - src_entropy_loss + tgt_entropy_loss
    ...
    def get_parameter_list(self):
        ...
        entropy_parameter_list = [
                {'params': self.f_net.parameters(), 'lr': 1},
                #{'params': self.c_net_a.parameters(), 'lr': 10},
                #{'params': self.c_net_b.parameters(), 'lr': 10},
                ]
        return classifier_parameter_list, entropy_parameter_list
