...
class XDA(object):
    def __init__(self, class_num=31, alpha=1.0, beta=1.0, gamma=1.0, use_gpu=False, writer=None):
        ...
        self.tgt_centor = torch.zeros([self.class_num, self.class_num])
        self.tgt_counter = torch.ones([self.class_num])
        ...
        if self.use_gpu:
            ...
            self.tgt_centor = self.tgt_centor.cuda()
            self.tgt_counter = self.tgt_counter.cuda()
    ...
    def get_loss(self, inputs, labels_source, epoch):
        ...
        src_selection_a = torch.argmax(src_probabilities_a, dim=1).cuda()
        tgt_selection_a = torch.argmax(tgt_probabilities_a, dim=1).cuda()
        src_selection_b = torch.argmax(src_probabilities_b, dim=1).cuda()
        tgt_selection_b = torch.argmax(tgt_probabilities_b, dim=1).cuda()

        src_centor_b = torch.zeros([self.class_num, self.class_num]).cuda()
        src_counter_b = torch.ones([self.class_num]).cuda()
        tgt_centor_b = torch.zeros([self.class_num, self.class_num]).cuda()
        tgt_counter_b = torch.ones([self.class_num]).cuda()

        for b_i in range(batch_size):
            self.src_centor[src_selection_a[b_i]] += src_probabilities_a[b_i].detach()
            self.src_counter[src_selection_a[b_i]] += 1
            self.tgt_centor[tgt_selection_a[b_i]] += tgt_probabilities_a[b_i].detach()
            self.tgt_counter[tgt_selection_a[b_i]] += 1
            src_centor_b[src_selection_b[b_i]] += src_probabilities_b[b_i]
            src_counter_b[src_selection_b[b_i]] += 1
            tgt_centor_b[tgt_selection_b[b_i]] += tgt_probabilities_b[b_i]
            tgt_counter_b[tgt_selection_b[b_i]] += 1

        src_curr_centor = torch.transpose(self.src_centor.transpose(0,1).detach() / self.src_counter.float(), 0, 1)
        tgt_curr_centor = torch.transpose(self.tgt_centor.transpose(0,1).detach() / self.tgt_counter.float(), 0, 1)
        src_curr_centor_b = torch.transpose(src_centor_b.transpose(0,1) / src_counter_b.float(), 0, 1)
        tgt_curr_centor_b = torch.transpose(tgt_centor_b.transpose(0,1) / tgt_counter_b.float(), 0, 1)

        src_centor_loss = self.beta * distance_criterion(src_curr_centor, src_curr_centor_b) / (self.class_num * self.class_num)
        tgt_centor_loss = self.beta * distance_criterion(tgt_curr_centor, tgt_curr_centor_b) / (self.class_num * self.class_num)
        ...
        self.writer.add_scalars('data/distance', {
            ...
            'src_centor_loss': src_centor_loss,
            'tgt_centor_loss': tgt_centor_loss,
            }, epoch)
        return classifier_loss, torch.abs(tgt_scale_loss - src_scale_loss + src_centor_loss - tgt_centor_loss)
