...
class XDA(object):
    ...
    def get_centor_loss(self, inputs_list, epoch):
        ...
        src_centor = torch.zeros([self.class_num]).cuda()
        tgt_centor = torch.zeros([self.class_num]).cuda()
        for inputs in inputs_list:
            high_features = self.f_net(inputs)
            src_high_features = high_features.narrow(0, 0, batch_size)
            tgt_high_features = high_features.narrow(0, batch_size, batch_size)
    
            _, src_probabilities_a = self.c_net_a(src_high_features)
            _, tgt_probabilities_a = self.c_net_a(tgt_high_features)
    
            src_centor += src_probabilities_a.mean(dim=0)
            tgt_centor += tgt_probabilities_a.mean(dim=0)

        src_curr_centor = src_centor / (batch_size * len(inputs_list))
        tgt_curr_centor = tgt_centor / (batch_size * len(inputs_list))
        centor_loss = distance_criterion(src_curr_centor, tgt_curr_centor) / self.class_num
    ...
    def get_parameter_list(self):
        ...
        centor_parameter_list = [
                {'params': self.f_net.parameters(), 'lr': 1},
                ]

        return classifier_parameter_list, scale_parameter_list, centor_parameter_list
