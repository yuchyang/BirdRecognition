...
class MCD(object):
    ...
    def get_C_loss(self, target_inputs, epoch, i):
        batch_size = target_inputs.size(0)
        distance_criterion = nn.L1Loss()

        tgt_high_features = self.f_net(target_inputs)
        ...
        self.writer.add_scalars('probabilities', {
            'tgt-a-max-' + str(i): torch.max(tgt_probabilities_a, 1)[0].mean(),
            'tgt-b-max-' + str(i): torch.max(tgt_probabilities_b, 1)[0].mean(),
            }, epoch)
        self.writer.add_scalars('data/loss', {
            'tgt_distance_loss-'+str(i): tgt_distance_loss,
            }, epoch)
        return tgt_distance_loss
    ...
    def get_parameter_list(self):
        ...
        distance_min_parameter_list = [
                {'params': self.f_net.parameters(), 'lr': 1},
                #{'params': self.c_net_a.parameters(), 'lr': 10},
                #{'params': self.c_net_b.parameters(), 'lr': 10},
                ]
        distance_max_parameter_list = [
                #{'params': self.f_net.parameters(), 'lr': 1},
                {'params': self.c_net_a.parameters(), 'lr': 10},
                {'params': self.c_net_b.parameters(), 'lr': 10},
                ]

        return classifier_parameter_list, distance_min_parameter_list, distance_max_parameter_list
