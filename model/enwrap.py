...
def _kl_div(inputs, probs):
    inputs = torch.clamp(inputs, 0.000001)
    inputs = torch.log(inputs)
    # pytorch KLDLoss is averaged over all dim if size_average=True
    kld = F.kl_div(inputs, probs, size_average=False)
    return kld / inputs.shape[0]
...
class Enwrap(object):
    ...
    def get_loss(self, inputs, labels_source, epoch):
        ...
        #vat_loss_a = vat_Loss(self.c_net_a, high_features, xi=10.0, eps=1.0, ip=1)
        #classifier_loss_b = class_criterion(src_outputs_b, labels_source)
        vat_loss_b = vat_Loss(self.c_net_b, high_features.detach(), xi=10.0, eps=1.0, ip=1)
        classifier_loss = classifier_loss_a
        vat_loss_ab = vat_loss_b
        # classifier A is trained by src
        ...
        self.writer.add_scalars('data/loss', {
            ...
            #'vat_loss_a': vat_loss_a,
            'vat_loss_b': vat_loss_b,
        ...
        return classifier_loss + vat_loss_ab * self.gamma, self.alpha * torch.abs(tgt_scale_loss - src_scale_loss)
    ...
    def predict(self, inputs):
        ...
        #softmax_outputs = softmax_outputs_a + softmax_outputs_b
